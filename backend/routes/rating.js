...
ratingRouter.post("/:recipeId/:value", async (req, res) => {
  ...
  const currentRating = db.getRatingByUserAndRecipeId(req.user.id, data.recipeId);
...
ratingRouter.delete("/:recipeId", async (req, res) => {
  ...
  const currentRating = db.getRatingByUserAndRecipeId(req.user.id, idParseResult.data);
  ...
  if (db.deleteRating(currentRating.id)) {
    res.sendStatus(200);
    return;
  }
